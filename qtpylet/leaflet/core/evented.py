...
class Evented(QObject):
    '''
    Base class for all qtpylet objects.
    ...
    def __init__(self, mapWidget=None):
        ...
        The first qtpylet object to be initiated should be qtpylet.L.map
        This will allow all the qtpylet objects to have access to the
        ...
        if mapWidget is None:
            raise RuntimeError('L.map must be initialised before other qtpylet objects')
        if not issubclass(type(mapWidget), mapwidget.MapWidget):
            raise TypeError(('Expected mapWidget of type qtpylet.MapWidget, '
